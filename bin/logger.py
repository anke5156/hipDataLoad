...
class Logger(object):
    ...
    def __init__(self, filename='../logs/hipdataload.log', level='info', when='D', backCount=3):
        ...
        # when是时间间隔，backupCount是备份文件的个数，如果超过这个个数，就会自动删除，when是间隔的时间单位，单位有以下几种：
        # S 秒
        # M 分
        # H 小时、
        # D 天、
...
if __name__ == '__main__':
    log = Logger('../logs/test.log', level='info').logger
    log.debug('debug')
    log.info('info')
    log.warning('警告')
    log.error('报错')
